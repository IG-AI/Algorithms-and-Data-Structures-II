...
def min_difference_aux(u,lenu,r,lenr,R):
    for i in range(lenu+1):
        for j in range(lenr+1):
            if i == 0 and j == 0:
                A[0][0] = min(R[u[i]][r[j]], R[u[i]]['-'] + R['-'][r[j]])

                '''if lenr == 0 and lenu == 0:
                return min(R[u[lenu]][r[lenr]], R[u[lenu]]['-'] + R['-'][r[lenr]])'''
            elif j == 0:
                sum = 0
                for k in range(i+1):
                    sum = sum + R[u[k]]['-']
                ...
                var = 0
                for k in range(i+1):
                    var = sum + R[u[k]][r[j]] - R[u[k]]['-']
                    if var < minn: 
                        minn = var
                A[i][0] = min(sum + R['-'][r[j]],minn)
                ...
                var = 0
                for i in range(lenu+1):
                    var = sum + R[u[i]][r[lenr]] - R[u[i]]['-']
                    if var < minn: 
                        minn = var
                return min(sum + R['-'][r[lenr]],minn)'''
                ...
                var = 0
                for k in range(j+1):
                    var = sum + R[u[i]][r[k]] - R['-'][r[k]]
                    if var < minn: 
                        minn = var
                A[0][j] = min(sum + R[u[i]]['-'],minn)
                ...
                var = 0
                for i in range(lenr+1):
                    var = sum + R[u[lenu]][r[i]] - R['-'][r[i]]
                    if var < minn: 
                        minn = var
                return min(sum + R[u[lenu]]['-'],minn)'''
                ...
                if A[i-1][j-1] + min(R[u[i]][r[j]], R['-'][r[j]] + R[u[i]]['-']) <= min(A[i-1][j] + R[u[i]]['-'], A[i][j-1] + R['-'][r[j]]):
                    V[i][j] = 'diag'

                elif A[i-1][j] + R[u[i]]['-'] <= min(A[i][j-1] + R['-'][r[j]], A[i-1][j-1] + min(R[u[i]][r[j]], R['-'][r[j]] + R[u[i]]['-'])):
                    V[i][j] = 'up'
